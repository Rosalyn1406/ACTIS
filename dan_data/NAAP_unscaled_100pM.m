...
% display equation
disp(['y=' , num2str(p3(1)) , '*x^3' , num2str(p3(2)) , '*x^2' , num2str(p3(3)) , '*x+', num2str(p3(4))])
...
hold off;
legend('Data','fit(3rd Degree)','Linear fit','Linewidth',1.0,'AutoUpdate','off')
grid on 
xlim([0 2.5])
title("R vs [T]_0 for K_d=60pM, NAAP Unscaled=100pM")
xlabel("[T]_{0}/K_{d}")
ylabel("R_{100PM}")
...
% Display equation
disp(['dy/dx = ' , num2str(deri(1)) , '*x^2' , num2str(deri(2)) , '*x' ...
    , num2str(deri(3))])
...
%% Roots
p =[  0.1848   -0.5784    0.0189    0.4271]
r=roots(p)

% sub x value into derivative to check if slope matches with y_p1_slope
slope=polyval(deri, 1.0897) % 0.0504 is the x value for y =0.5
